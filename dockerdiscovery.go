...
type DockerDiscovery struct {
	...
	filterNetwork  string // network to filter containers by (for filter_network option)
...
func (dd *DockerDiscovery) getContainerAddress(container *dockerapi.Container, v6 bool) (net.IP, error) {
	...
	// check for label-based network override (highest priority)
	...
	if hasNetName {
		...
		if !ok {
			return nil, fmt.Errorf("unable to find network settings for the network %s", netName)
		}
		log.Printf("[docker] network name %s specified via label (%s)", netName, container.ID[:12])
		if !v6 {
	...
	// if filterNetwork is set (via filter_network option), use it
	if dd.filterNetwork != "" {
		network, ok := container.NetworkSettings.Networks[dd.filterNetwork]
		if !ok {
			// container is not in the filter network, skip it
			return nil, nil
		}

		// use IP from the filter network
		if !v6 {
			return net.ParseIP(network.IPAddress), nil
		} else if v6 && len(network.GlobalIPv6Address) > 0 {
			return net.ParseIP(network.GlobalIPv6Address), nil
	...
	for {
		if container.NetworkSettings.IPAddress != "" && !v6 {
			return net.ParseIP(container.NetworkSettings.IPAddress), nil
		}

		if container.NetworkSettings.GlobalIPv6Address != "" && v6 {
