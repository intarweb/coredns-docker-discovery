# Fork tracking — intarweb/coredns-docker-discovery

This is a soft fork of [`vxcontrol/coredns-docker-discovery`](https://github.com/vxcontrol/coredns-docker-discovery). We track upstream HEAD and publish a HEAD-tracked image to GHCR with any open intarweb→upstream PRs cherry-picked on top.
...
| Property | Value |
|---|---|
| Upstream | [`vxcontrol/coredns-docker-discovery`](https://github.com/vxcontrol/coredns-docker-discovery) |
...
| Property | Value |
|---|---|
| Image | `ghcr.io/intarweb/coredns-docker-discovery` |
...
services:
  coredns-docker-discovery:
    image: ghcr.io/intarweb/coredns-docker-discovery:latest
```

Pin to a specific build for reproducibility:

```yaml
    image: ghcr.io/intarweb/coredns-docker-discovery:sha-<short>
```
...
```bash
gh workflow run "Sync from upstream + auto-regen intarweb-dev" --repo intarweb/coredns-docker-discovery
```

**Force a fresh build of `:latest` without an upstream change:**
```bash
gh workflow run "Build from source → GHCR" --repo intarweb/coredns-docker-discovery --ref intarweb-dev
```
...
gh pr create --repo vxcontrol/coredns-docker-discovery --base main --head intarweb:feat/my-thing
